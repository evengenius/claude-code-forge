...
│   │   ├── plan.md                #   /plan — планирование задачи
│   │   ├── implement.md           #   /implement — TDD-реализация
│   │   ├── handoff.md             #   /handoff — передача контекста
│   │   ├── review.md              #   /review — code review
│   │   ├── freshstart.md          #   /freshstart — начало сессии
│   │   ├── compact-save.md        #   /compact-save — безопасная компакция
│   │   └── security-audit.md      #   /security-audit — аудит безопасности
...
│   └── _template.md               #   Шаблон ADR (нумерация начинается с 001)
├── _todo/                         # Handoff-документы и TPP
├── _done/                         # Завершённые handoff (в .gitignore)
└── .claude/commands/
    └── adr.md                     #   /adr — создание ADR
```
...
│   └── commands/
│       └── gate-check.md          #   /gate-check — проверка Quality Gate
└── specs/                         # Спецификации
```
...
**Зависимость**: хуки парсят stdin через `jq` (`apt install jq` / `brew install jq`). Без `jq` хуки молча отрабатывают как no-op — скрипт инициализации это проверяет и предупреждает.

**Stop** (по умолчанию выключен): можно добавить автозапуск тест-сьюта, чтобы Claude получал результаты падающих тестов и продолжал работу. Не включён по умолчанию, потому что полный прогон тестов на каждом завершении сессии может быть медленным или иметь побочные эффекты (БД, сеть, контейнеры).
...
- `/compact-save` — сохранить важное перед компакцией
...
Forge навязывает Test-Driven Development через команду `/implement` и правила в `.claude/rules/testing.md`:
...
|---------|----------|-------------------|
| `/freshstart` | Загрузить весь контекст проекта, показать статус и рекомендовать следующую задачу | В начале каждой рабочей сессии |
| `/plan [задача]` | Спланировать реализацию с учётом ADR и зависимостей | Перед началом любой задачи >30 мин |
| `/implement [задача]` | Реализовать по TDD-циклу (RED→GREEN→REFACTOR) | Непосредственно реализация |
| `/review [scope]` | Code review с классификацией замечаний (🔴🟡🔵) | После завершения фичи, перед мержем |
| `/handoff` | Создать handoff-документ для следующей сессии | В конце каждой рабочей сессии |
| `/compact-save` | Сохранить важный контекст перед компакцией | При заполнении контекста на 60–80% |
| `/adr [тема]` | Создать Architecture Decision Record | При принятии архитектурного решения |
| `/security-audit [scope]` | Проверить секреты, SQL injection, XSS, зависимости | Периодически и перед релизом |
| `/gate-check` | Проверка Quality Gate (90/100) | Перед переходом к реализации (Full) |
...
Утро:
  ...
  /plan [задача]           ← спланировать в Plan Mode

Работа:
  /implement [задача]      ← TDD-реализация
  ...при 60-80% контекста...
  /compact-save            ← сохранить перед компакцией
